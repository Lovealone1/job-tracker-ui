...
import { CrudEntityConfig, TableColumn } from '@/types/crud';
import { useCrudList, useCrudCreate, useCrudUpdate, useCrudToggle, useCrudDelete } from '@/hooks/use-crud';
import { CrudTable } from './crud-table';
import { CrudFormDialog } from './crud-form-dialog';
import { CrudConfirmDialog } from './crud-confirm-dialog';
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-[#A600FF] dark:text-[#C966FF] mt-1 font-medium">
                        Gestiona los registros de {config.title.toLowerCase()} en el sistema.
                    </p>
                </div>
                <button
                    onClick={handleOpenCreate}
                    className="flex items-center gap-2 px-4 py-2 bg-[#A600FF] hover:bg-[#8B00D6] text-white rounded-lg text-sm font-bold shadow-sm transition-colors"
                >
                    <Plus size={16} />
